...
function Clock() {
    ...
    const stringTime =
        `${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())}` // часы24:минуты:секунды (01:02:03)
    const stringDate =
        `${pad(date.getDate())}.${pad(date.getMonth() + 1)}.${date.getFullYear()}` // день.месяц.год (01.02.2022)
